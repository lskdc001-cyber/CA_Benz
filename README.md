...
| 콘텐츠 생성 (`/content`) | 차종·프로모션 조건을 입력하면 인스타그램/블로그/카카오채널 문구를 자동 생성 |
...
- 광고성 정보 수신동의(정보통신망법)는 CRM 등록 시 수집하며, 동의 없는 채널의 광고성 사후관리 메시지는 자동으로 `발송 불가` 처리됩니다. 실발송 연동 시 본문 `(광고)` 표기·수신거부 방법 명시·2년 주기 동의 재확인을 추가 구현해야 합니다.
